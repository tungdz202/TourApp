import { createSlice } from "@reduxjs/toolkit";

const CollectionSlice = createSlice({
  name: "Collection",
  initialState: {
    collection: {
      collection: [
        {
          _id: "",
          name: "",
          price: 0,
          priceMin: 0,
          priceMax: 0,
          imgs: [
            "",
            "https://scontent.fhan2-4.fna.fbcdn.net/v/t39.30808-6/353394623_293836892999920_8252722835748439789_n.jpg?_nc_cat=100&ccb=1-7&_nc_sid=8bfeb9&_nc_ohc=lyRipw-9uA8AX-sOLjt&_nc_ht=scontent.fhan2-4.fna&oh=00_AfBMQrtT4z1_bzNZHj2D2MlefpJFL7M-fKEjfYUQJ-J4AQ&oe=649DBCF5",
          ],
          departurePoint: "",
          listTour: [
            {
              _id: "",
              name: "",
              imgs: [
                "",
                "https://scontent.fhan2-4.fna.fbcdn.net/v/t39.30808-6/353394623_293836892999920_8252722835748439789_n.jpg?_nc_cat=100&ccb=1-7&_nc_sid=8bfeb9&_nc_ohc=lyRipw-9uA8AX-sOLjt&_nc_ht=scontent.fhan2-4.fna&oh=00_AfBMQrtT4z1_bzNZHj2D2MlefpJFL7M-fKEjfYUQJ-J4AQ&oe=649DBCF5",
              ],
              url: "",
              originalPrice: 0,
              presentPrice: 0,
              time: "",
              vehicle: "",
              schedules: [" "],
              highlightDestinations: "",
            },
          ],
        },
      ],
      isFetching: false,
      errors: false,
    },
  },
  reducers: {
    getCollectionStart: (state) => {
      state.collection.isFetching = true;
    },
    getCollectionSuccess: (state, action) => {
      state.collection.isFetching = false;
      state.collection.collection = action.payload;
      state.collection.errors = false;
    },
    getCollectionFailed: (state) => {
      state.collection.isFetching = false;
      state.collection.errors = true;
    },
  },
});

export const { getCollectionStart, getCollectionSuccess, getCollectionFailed } =
  CollectionSlice.actions;
export default CollectionSlice.reducer;
